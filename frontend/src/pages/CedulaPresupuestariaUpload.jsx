...
import { Upload, FileText, CheckCircle, AlertCircle, FileSpreadsheet, List, DollarSign } from "lucide-react"
...
export default function CedulaPresupuestariaUpload() {
  ...
  const fetchSummary = async () => {
    try {
      ...
      const res = await fetch("http://localhost:8000/api/cedula-presupuestaria/summary", {
  ...
  const uploadFile = async (fileToUpload) => {
    ...
    try {
      ...
      const token = Cookies.get("auth_token")
      const res = await fetch("http://localhost:8000/api/cedula-presupuestaria/upload", {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      })
  ...
  const summaryItems = summary ? [
    { label: 'Registros',       value: summary.registros_count      || 0, icon: <List size={18} />,            color: ACCENT },
    { label: 'Certificaciones', value: summary.certificaciones_count || 0, icon: <FileSpreadsheet size={18} />, color: theme.colors.accent.green },
    { label: 'Monto Total',     value: '$' + (summary.monto_total   || 0), icon: <DollarSign size={18} />,      color: theme.colors.accent.gold },
  ] : []
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: theme.border.radiusMd, padding: '24px', marginBottom: '20px' }}>
        {!showUploadForm ? (
          <div style={{ textAlign: 'center', padding: '12px 0' }}>
            ...
            <p style={{ margin: '0 0 20px', fontSize: '13px', color: MUTED }}>Carga un archivo CSV con la cédula presupuestaria</p>
            <button
          ...
          <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <label htmlFor="cedula-file-input" style={{
              border: `2px dashed ${ACCENT}55`, borderRadius: theme.border.radiusMd,
              padding: '28px 20px', textAlign: 'center', background: `${ACCENT}08`,
